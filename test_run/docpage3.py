...
# INJECT THE ADMINPAGE TABS DESIGN
st.markdown(f"""
    <style>
    {theme_css}
...
with col_main:
    ...
    with tab_queue:
        ...
        # --- 1. TODAY'S ACTIVE TRIAGE QUEUE (HIGHLIGHTED CARDS) ---
        ...
        with st.container(border=True):
            ...
            with f_c3: 
                modality_view = st.selectbox("Chronic Modality Switcher", ["Cardiovascular (CVD)", "Type 2 Diabetes", "COPD (Pulmonary)", "Mental Health / Neuro"], label_visibility="collapsed", key="modality_switcher")

            for pat in master_patients:
                pat.setdefault("fasting_glucose", 142)
                pat.setdefault("hba1c", 7.8)
                pat.setdefault("spo2", 95)
                pat.setdefault("fev1_fvc", "72%")
                pat.setdefault("pack_years", 15)
                pat.setdefault("phq9", 6)
                pat.setdefault("gad7", 4)
                pat.setdefault("sleep_hrs", 5.5)
                pat.setdefault("allergies", "Penicillin, Sulfa drugs")
                pat.setdefault("active_rx", "Amlodipine 10mg OD, Metformin 500mg BID")
            ...
            with enq_c1:
                if sel_dir_rows:
                    ...
                    st.markdown(f"""
                    <div style="background: rgba(0, 121, 121, 0.05); border: 1px solid rgba(0, 121, 121, 0.2); border-radius: 8px; padding: 12px; margin-top: 10px;">
                        ...
                        <div style="display: flex; gap: 18px; font-size: 0.85rem;">
                            <div><b>Active Rx:</b> {selected_dir_patient['active_rx']}</div>
                            <div><b>Allergies:</b> <span style="color:#ef4444; font-weight:700;">{selected_dir_patient['allergies']}</span></div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.caption("*(Select a patient row to preview records and unlock enqueue actions)*")

            with enq_c2:
                st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
                if st.button("➕ Enqueue Selected Patient to Triage", type="primary", use_container_width=True, disabled=not bool(sel_dir_rows)):
                    if sel_dir_rows:
        ...
        # --- 2.5 FAMILY HISTORY & HEREDITARY RISK DIRECTORY TABLE ---
        with st.container(border=True):
            ...
            # Mock family history registry records
            family_history_data = [
                {"id": "PAT-9912", "name": "Alvarez, Carlos T.", "modality": "Cardiovascular", "lineage": "Father: Early CVD <55 yrs | Mother: Hypertension", "onset": "Early Onset (<55 yrs)", "risk_status": "🔴 High Genetic Load"},
                {"id": "PAT-4421", "name": "Santos, Sofia M.", "modality": "Diabetes Lineage", "lineage": "Mother: Type 2 Diabetes (Diagnosed at 48)", "onset": "Adult Onset (>40 yrs)", "risk_status": "🟡 Moderate"},
                {"id": "PAT-1102", "name": "Mason, Justin L.", "modality": "Pulmonary/Asthma History", "lineage": "None Recorded", "onset": "N/A", "risk_status": "🟢 Low Risk"},
            ]
        ...
        # --- 3. LONGITUDINAL TIMELINE & CLINICAL HISTORY EXPANDER ---
...
# ==============================================================================
# CDSS PARAMETER INGESTION ENGINE (TAB 3: AI DIAGNOSTICS & CDSS)
# ==============================================================================
...
with tab_cdss:
    ...
    # --------------------------------------------------------------------------
    # PATIENT IDENTITY ENFORCEMENT & CONTEXT INITIALIZATION
    # --------------------------------------------------------------------------
    if not st.session_state.get("active_patient") and "master_patients" in globals() and master_patients:
        st.session_state.active_patient = master_patients[0]
        
    active_pat = st.session_state.get("active_patient", {
        "id": "PAT-9912",
        "name": "Alvarez, Carlos T.",
        "age": 58,
        "sex": "Male",
        "bmi": 29.4,
        "latest_bp": "165/95",
    ...
    # Header Consultation Context
    with st.container(border=True):
        p_c1, p_c2, p_c3, p_c4, p_c5 = st.columns(5)
        p_c1.metric("Patient ID", active_pat["id"])
        p_c2.metric("Full Name", active_pat["name"])
        p_c3.metric("Chronological Age", f"{active_pat['age']} Yrs")
        p_c4.metric("Biological Sex", active_pat["sex"])
        p_c5.metric("Baseline Vitals", f"{active_pat.get('latest_bp', '120/80')} | {active_pat.get('resting_hr', 75)} bpm")
    ...
    # Modality Normalization for State, Forms, and JSON Schema
    modality_dispatch = {
    ...
    # --------------------------------------------------------------------------
    # MODALITY FORM INGESTION CONTAINER
    # --------------------------------------------------------------------------
    with st.form(key=f"cdss_{modality_key}_form", clear_on_submit=False):
        st.markdown(f"#### Clinical Parameter Ingestion — {modality_schema_name}")
        ...
        # ======================================================================
        # MODALITY 1: CARDIOVASCULAR DISEASES (HYPERTENSION / CVD)
        # ======================================================================
        if modality_key == "cardiovascular":
            with st.expander("🩺 Hemodynamics & Anthropometrics", expanded=True):
                cvd_age = st.number_input(
                    "Age", min_value=18, max_value=90, value=int(active_pat.get("age", 58)), step=1, key="cvd_age"
                )
                st.caption("Expected Clinical Range: 18 – 90 Years")
                payload_buffer["Age"] = {"raw_value": cvd_age, "unit": "Years"}

                cvd_sex = st.selectbox(
                    "Biological Sex", options=["Female", "Male"], 
                    index=1 if active_pat.get("sex") == "Male" else 0, key="cvd_sex"
                )
                st.caption("Reference: Female / Male")
                payload_buffer["Biological Sex"] = {"raw_value": cvd_sex, "unit": "None"}

                cvd_sbp = st.number_input(
                    "Systolic Blood Pressure (SBP)", min_value=70.0, max_value=220.0, 
                    value=float(active_pat.get("latest_bp", "120/80").split("/")[0]), step=1.0, format="%.1f", key="cvd_sbp"
                )
                st.caption("Expected Clinical Range: 70 – 220 mmHg")
                payload_buffer["Systolic Blood Pressure (SBP)"] = {"raw_value": cvd_sbp, "unit": "mmHg"}

                cvd_dbp = st.number_input(
                    "Diastolic Blood Pressure (DBP)", min_value=40.0, max_value=130.0, 
                    value=float(active_pat.get("latest_bp", "120/80").split("/")[1]), step=1.0, format="%.1f", key="cvd_dbp"
                )
                st.caption("Expected Clinical Range: 40 – 130 mmHg")
                payload_buffer["Diastolic Blood Pressure (DBP)"] = {"raw_value": cvd_dbp, "unit": "mmHg"}

                cvd_hr = st.number_input(
                    "Heart Rate (Resting)", min_value=40, max_value=180, 
                    value=int(active_pat.get("resting_hr", 75)), step=1, key="cvd_hr"
                )
                st.caption("Expected Clinical Range: 40 – 180 bpm")
                payload_buffer["Heart Rate (Resting)"] = {"raw_value": cvd_hr, "unit": "bpm"}

                cvd_bmi = st.number_input(
                    "Body Mass Index (BMI)", min_value=12.0, max_value=60.0, 
                    value=float(active_pat.get("bmi", 26.5)), step=0.1, format="%.1f", key="cvd_bmi"
                )
                st.caption("Expected Clinical Range: 12.0 – 60.0 kg/m²")
                payload_buffer["Body Mass Index (BMI)"] = {"raw_value": cvd_bmi, "unit": "kg/m²"}

            with st.expander("🧪 Serum Lipid, Inflammatory & Renal Panels", expanded=True):
                cvd_tc = st.number_input(
                    "Total Cholesterol", min_value=100.0, max_value=400.0, 
                    value=float(active_pat.get("total_chol", 200.0)), step=1.0, format="%.1f", key="cvd_tc"
                )
                st.caption("Expected Clinical Range: 100 – 400 mg/dL")
                payload_buffer["Total Cholesterol"] = {"raw_value": cvd_tc, "unit": "mg/dL"}

                cvd_hdl = st.number_input(
                    "High-Density Lipoprotein (HDL)", min_value=15.0, max_value=120.0, value=45.0, step=1.0, format="%.1f", key="cvd_hdl"
                )
                st.caption("Expected Clinical Range: 15 – 120 mg/dL")
                payload_buffer["High-Density Lipoprotein (HDL)"] = {"raw_value": cvd_hdl, "unit": "mg/dL"}

                cvd_ldl = st.number_input(
                    "Low-Density Lipoprotein (LDL)", min_value=30.0, max_value=300.0, value=130.0, step=1.0, format="%.1f", key="cvd_ldl"
                )
                st.caption("Expected Clinical Range: 30 – 300 mg/dL")
                payload_buffer["Low-Density Lipoprotein (LDL)"] = {"raw_value": cvd_ldl, "unit": "mg/dL"}

                cvd_trig = st.number_input(
                    "Serum Triglycerides", min_value=40.0, max_value=1000.0, value=160.0, step=1.0, format="%.1f", key="cvd_trig"
                )
                st.caption("Expected Clinical Range: 40 – 1000 mg/dL")
                payload_buffer["Serum Triglycerides"] = {"raw_value": cvd_trig, "unit": "mg/dL"}

                cvd_hscrp = st.number_input(
                    "High-Sensitivity C-Reactive Protein (hs-CRP)", min_value=0.1, max_value=50.0, value=2.5, step=0.1, format="%.1f", key="cvd_hscrp"
                )
                ...
                cvd_creat = st.number_input(
                    "Serum Creatinine", min_value=0.4, max_value=10.0, value=1.00, step=0.01, format="%.2f", key="cvd_creat"
                )
                st.caption("Expected Clinical Range: 0.4 – 10.0 mg/dL")
                payload_buffer["Serum Creatinine"] = {"raw_value": cvd_creat, "unit": "mg/dL"}

                cvd_egfr = st.number_input(
                    "Estimated Glomerular Filtration Rate (eGFR)", min_value=5.0, max_value=140.0, value=85.0, step=1.0, format="%.1f", key="cvd_egfr"
                )
            ...
            with st.expander("🫀 Cardiac Necrosis, Remodeling & Myocardial Function", expanded=True):
                cvd_lvef = st.number_input(
                    "Left Ventricular Ejection Fraction (LVEF)", min_value=15.0, max_value=75.0, value=55.0, step=1.0, format="%.1f", key="cvd_lvef"
                )
                st.caption("Expected Clinical Range: 15 – 75 %")
                payload_buffer["Left Ventricular Ejection Fraction (LVEF)"] = {"raw_value": cvd_lvef, "unit": "%"}

                cvd_ctni = st.number_input(
                    "Cardiac Troponin I (cTnI)", min_value=0.01, max_value=50.0, value=0.02, step=0.01, format="%.2f", key="cvd_ctni"
                )
                st.caption("Expected Clinical Range: 0.01 – 50.0 ng/mL")
                payload_buffer["Cardiac Troponin I (cTnI)"] = {"raw_value": cvd_ctni, "unit": "ng/mL"}

                cvd_ntpro = st.number_input(
                    "N-Terminal Pro-B-Type Natriuretic Peptide (NT-proBNP)", min_value=10.0, max_value=35000.0, value=95.0, step=10.0, format="%.1f", key="cvd_ntpro"
                )
            ...
            with st.expander("🏃 Behavioral Risk Factors & Hereditary Predisposition", expanded=True):
                cvd_smoke = st.number_input(
                    "Smoking History", min_value=0.0, max_value=120.0, value=10.0, step=1.0, format="%.1f", key="cvd_smoke"
                )
                st.caption("Expected Clinical Range: 0 – 120 Pack-Years")
                payload_buffer["Smoking History"] = {"raw_value": cvd_smoke, "unit": "Pack-Years"}

                cvd_sodium = st.number_input(
                    "Daily Sodium Intake", min_value=500.0, max_value=10000.0, value=2500.0, step=50.0, format="%.1f", key="cvd_sodium"
                )
                st.caption("Expected Clinical Range: 500 – 10,000 mg/day")
                payload_buffer["Daily Sodium Intake"] = {"raw_value": cvd_sodium, "unit": "mg/day"}

                cvd_famhist = st.radio(
                    "Family History of Premature CVD", options=["0", "1"], index=0, horizontal=True, key="cvd_famhist"
                )
                ...
                cvd_activity = st.number_input(
                    "Physical Activity Level", min_value=0, max_value=1050, value=150, step=15, key="cvd_activity"
                )
        ...
        # ======================================================================
            ...
            with st.expander("🩸 Glycemic Status & Endocrine Regulation", expanded=True):
                dia_fpg = st.number_input(
                    "Fasting Plasma Glucose (FPG)", min_value=50.0, max_value=450.0, value=115.0, step=1.0, format="%.1f", key="dia_fpg"
                )
                st.caption("Expected Clinical Range: 50 – 450 mg/dL")
                payload_buffer["Fasting Plasma Glucose (FPG)"] = {"raw_value": dia_fpg, "unit": "mg/dL"}

                dia_2hpg = st.number_input(
                    "2-Hour Postprandial Glucose (2h-PG)", min_value=60.0, max_value=600.0, value=155.0, step=1.0, format="%.1f", key="dia_2hpg"
                )
                ...
                dia_hba1c = st.number_input(
                    "Glycated Hemoglobin (HbA1c)", min_value=4.0, max_value=15.0, value=6.5, step=0.1, format="%.1f", key="dia_hba1c"
                )
                st.caption("Expected Clinical Range: 4.0 – 15.0 %")
                payload_buffer["Glycated Hemoglobin (HbA1c)"] = {"raw_value": dia_hba1c, "unit": "%"}

                dia_ins = st.number_input(
                    "Fasting Serum Insulin", min_value=1.0, max_value=100.0, value=15.0, step=0.5, format="%.1f", key="dia_ins"
                )
                st.caption("Expected Clinical Range: 1.0 – 100.0 µIU/mL")
                payload_buffer["Fasting Serum Insulin"] = {"raw_value": dia_ins, "unit": "µIU/mL"}

                dia_cpep = st.number_input(
                    "Serum C-Peptide Level", min_value=0.1, max_value=12.0, value=2.4, step=0.1, format="%.1f", key="dia_cpep"
                )
                st.caption("Expected Clinical Range: 0.1 – 12.0 ng/mL")
                payload_buffer["Serum C-Peptide Level"] = {"raw_value": dia_cpep, "unit": "ng/mL"}

                dia_homa = st.number_input(
                    "HOMA-IR (Insulin Resistance Score)", min_value=0.2, max_value=25.0, value=2.80, step=0.1, format="%.2f", key="dia_homa"
                )
            ...
            with st.expander("📏 Anthropometrics & Central Adiposity", expanded=True):
                dia_bmi = st.number_input(
                    "Body Mass Index (BMI)", min_value=12.0, max_value=60.0, 
                    value=float(active_pat.get("bmi", 28.5)), step=0.1, format="%.1f", key="dia_bmi"
                )
                st.caption("Expected Clinical Range: 12.0 – 60.0 kg/m²")
                payload_buffer["Body Mass Index (BMI)"] = {"raw_value": dia_bmi, "unit": "kg/m²"}

                dia_waist = st.number_input(
                    "Waist Circumference", min_value=50.0, max_value=160.0, value=92.0, step=0.5, format="%.1f", key="dia_waist"
                )
                st.caption("Expected Clinical Range: 50 – 160 cm")
                payload_buffer["Waist Circumference"] = {"raw_value": dia_waist, "unit": "cm"}

                dia_whr = st.number_input(
                    "Waist-to-Hip Ratio (WHR)", min_value=0.60, max_value=1.40, value=0.88, step=0.01, format="%.2f", key="dia_whr"
                )
                st.caption("Expected Clinical Range: 0.60 – 1.40")
                payload_buffer["Waist-to-Hip Ratio (WHR)"] = {"raw_value": dia_whr, "unit": "Ratio"}

            with st.expander("🧪 Hemodynamics, Renal & Metabolic Markers", expanded=True):
                dia_sbp = st.number_input(
                    "Systolic Blood Pressure (SBP)", min_value=70.0, max_value=220.0, value=125.0, step=1.0, format="%.1f", key="dia_sbp"
                )
                st.caption("Expected Clinical Range: 70 – 220 mmHg")
                payload_buffer["Systolic Blood Pressure (SBP)"] = {"raw_value": dia_sbp, "unit": "mmHg"}

                dia_dbp = st.number_input(
                    "Diastolic Blood Pressure (DBP)", min_value=40.0, max_value=130.0, value=82.0, step=1.0, format="%.1f", key="dia_dbp"
                )
                st.caption("Expected Clinical Range: 40 – 130 mmHg")
                payload_buffer["Diastolic Blood Pressure (DBP)"] = {"raw_value": dia_dbp, "unit": "mmHg"}

                dia_trig = st.number_input(
                    "Serum Triglycerides", min_value=40.0, max_value=1000.0, value=175.0, step=1.0, format="%.1f", key="dia_trig"
                )
                st.caption("Expected Clinical Range: 40 – 1000 mg/dL")
                payload_buffer["Serum Triglycerides"] = {"raw_value": dia_trig, "unit": "mg/dL"}

                dia_hdl = st.number_input(
                    "High-Density Lipoprotein (HDL)", min_value=15.0, max_value=120.0, value=42.0, step=1.0, format="%.1f", key="dia_hdl"
                )
                st.caption("Expected Clinical Range: 15 – 120 mg/dL")
                payload_buffer["High-Density Lipoprotein (HDL)"] = {"raw_value": dia_hdl, "unit": "mg/dL"}

                dia_uacr = st.number_input(
                    "Urine Albumin-to-Creatinine Ratio (UACR)", min_value=1.0, max_value=3500.0, value=35.0, step=1.0, format="%.1f", key="dia_uacr"
                )
                ...
                dia_uric = st.number_input(
                    "Serum Uric Acid", min_value=1.5, max_value=14.0, value=6.2, step=0.1, format="%.1f", key="dia_uric"
                )
                st.caption("Expected Clinical Range: 1.5 – 14.0 mg/dL")
                payload_buffer["Serum Uric Acid"] = {"raw_value": dia_uric, "unit": "mg/dL"}

            with st.expander("🧬 Lineage, Clinical Presentation & Lifestyle", expanded=True):
                dia_gest = st.radio(
                    "Gestational Diabetes History", options=["0", "1"], index=0, horizontal=True, key="dia_gest"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Gestational Diabetes History"] = {"raw_value": dia_gest, "unit": "None"}

                dia_fam = st.radio(
                    "Family History of Diabetes", options=["0", "1"], index=1, horizontal=True, key="dia_fam"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Family History of Diabetes"] = {"raw_value": dia_fam, "unit": "None"}

                dia_acan = st.radio(
                    "Acanthosis Nigricans Presence", options=["0", "1"], index=0, horizontal=True, key="dia_acan"
                )
                st.caption("Input Basis: 0 = Absent, 1 = Present")
                payload_buffer["Acanthosis Nigricans Presence"] = {"raw_value": dia_acan, "unit": "None"}

                dia_carb = st.number_input(
                    "Daily Carbohydrate Intake", min_value=50.0, max_value=600.0, value=250.0, step=5.0, format="%.1f", key="dia_carb"
                )
                st.caption("Expected Clinical Range: 50 – 600 g/day")
                payload_buffer["Daily Carbohydrate Intake"] = {"raw_value": dia_carb, "unit": "g/day"}

                dia_sed = st.number_input(
                    "Sedentary Behavior Duration", min_value=1.0, max_value=18.0, value=7.0, step=0.5, format="%.1f", key="dia_sed"
                )
        ...
        # ======================================================================
            ...
            with st.expander("🫁 Spirometry, Volumes & Flow Dynamics", expanded=True):
                cr_fev1 = st.number_input(
                    "Forced Expiratory Volume in 1 Second (FEV1)", min_value=0.5, max_value=5.5, value=2.40, step=0.05, format="%.2f", key="cr_fev1"
                )
                ...
                cr_fvc = st.number_input(
                    "Forced Vital Capacity (FVC)", min_value=1.0, max_value=7.0, value=3.40, step=0.05, format="%.2f", key="cr_fvc"
                )
                st.caption("Expected Clinical Range: 1.0 – 7.0 Liters")
                payload_buffer["Forced Vital Capacity (FVC)"] = {"raw_value": cr_fvc, "unit": "Liters"}

                cr_rat = st.number_input(
                    "FEV1/FVC Ratio", min_value=25.0, max_value=95.0, value=70.5, step=0.1, format="%.1f", key="cr_rat"
                )
                st.caption("Expected Clinical Range: 25.0 – 95.0 %")
                payload_buffer["FEV1/FVC Ratio"] = {"raw_value": cr_rat, "unit": "%"}

                cr_pbr = st.number_input(
                    "Post-Bronchodilator FEV1 % Predicted", min_value=15.0, max_value=120.0, value=78.0, step=0.5, format="%.1f", key="cr_pbr"
                )
                st.caption("Expected Clinical Range: 15.0 – 120.0 %")
                payload_buffer["Post-Bronchodilator FEV1 % Predicted"] = {"raw_value": cr_pbr, "unit": "%"}

                cr_pef = st.number_input(
                    "Peak Expiratory Flow (PEF)", min_value=50.0, max_value=800.0, value=420.0, step=5.0, format="%.1f", key="cr_pef"
                )
                st.caption("Expected Clinical Range: 50 – 800 L/min")
                payload_buffer["Peak Expiratory Flow (PEF)"] = {"raw_value": cr_pef, "unit": "L/min"}

            with st.expander("🧪 Inflammatory Biomarkers & Arterial Blood Gases", expanded=True):
                cr_eos = st.number_input(
                    "Blood Eosinophil Count", min_value=0, max_value=2500, value=180, step=10, key="cr_eos"
                )
                st.caption("Expected Clinical Range: 0 – 2500 cells/µL")
                payload_buffer["Blood Eosinophil Count"] = {"raw_value": cr_eos, "unit": "cells/µL"}

                cr_feno = st.number_input(
                    "Fractional Exhaled Nitric Oxide (FeNO)", min_value=5.0, max_value=150.0, value=22.0, step=1.0, format="%.1f", key="cr_feno"
                )
                ...
                cr_spo2 = st.number_input(
                    "Resting Oxygen Saturation (SpO2)", min_value=65.0, max_value=100.0, value=96.0, step=0.1, format="%.1f", key="cr_spo2"
                )
                st.caption("Expected Clinical Range: 65.0 – 100.0 %")
                payload_buffer["Resting Oxygen Saturation (SpO2)"] = {"raw_value": cr_spo2, "unit": "%"}

                cr_pao2 = st.number_input(
                    "Arterial Oxygen Partial Pressure (PaO2)", min_value=40.0, max_value=110.0, value=85.0, step=1.0, format="%.1f", key="cr_pao2"
                )
                ...
                cr_paco2 = st.number_input(
                    "Arterial Carbon Dioxide Partial Pressure (PaCO2)", min_value=25.0, max_value=90.0, value=40.0, step=1.0, format="%.1f", key="cr_paco2"
                )
                ...
                cr_ige = st.number_input(
                    "Serum Total IgE Level", min_value=2.0, max_value=3000.0, value=85.0, step=10.0, format="%.1f", key="cr_ige"
                )
                st.caption("Expected Clinical Range: 2 – 3000 IU/mL")
                payload_buffer["Serum Total IgE Level"] = {"raw_value": cr_ige, "unit": "IU/mL"}

            with st.expander("📊 Functional Impairment, Control Scores & Exacerbations", expanded=True):
                cr_mmrc = st.selectbox(
                    "mMRC Dyspnea Scale Score", options=[0, 1, 2, 3, 4], index=1, key="cr_mmrc"
                )
                ...
                cr_act = st.number_input(
                    "Asthma Control Test (ACT) Score", min_value=5, max_value=25, value=21, step=1, key="cr_act"
                )
                st.caption("Expected Clinical Range: 5 – 25 Score")
                payload_buffer["Asthma Control Test (ACT) Score"] = {"raw_value": cr_act, "unit": "Score"}

                cr_cat = st.number_input(
                    "COPD Assessment Test (CAT) Score", min_value=0, max_value=40, value=8, step=1, key="cr_cat"
                )
                st.caption("Expected Clinical Range: 0 – 40 Score")
                payload_buffer["COPD Assessment Test (CAT) Score"] = {"raw_value": cr_cat, "unit": "Score"}

                cr_exac = st.number_input(
                    "Annual Exacerbation Frequency", min_value=0, max_value=15, value=0, step=1, key="cr_exac"
                )
                ...
                cr_tob = st.number_input(
                    "Lifetime Tobacco Exposure", min_value=0.0, max_value=120.0, value=5.0, step=1.0, format="%.1f", key="cr_tob"
                )
                st.caption("Expected Clinical Range: 0 – 120 Pack-Years")
                payload_buffer["Lifetime Tobacco Exposure"] = {"raw_value": cr_tob, "unit": "Pack-Years"}

            with st.expander("🏭 Occupational, Environmental & Atopic History", expanded=True):
                cr_dust = st.radio(
                    "Occupational Dust/Fume Exposure", options=["0", "1"], index=0, horizontal=True, key="cr_dust"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Occupational Dust/Fume Exposure"] = {"raw_value": cr_dust, "unit": "None"}

                cr_rhin = st.radio(
                    "Allergic Rhinitis Comorbidity", options=["0", "1"], index=0, horizontal=True, key="cr_rhin"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Allergic Rhinitis Comorbidity"] = {"raw_value": cr_rhin, "unit": "None"}

                cr_cast = st.radio(
                    "Childhood Asthma History", options=["0", "1"], index=0, horizontal=True, key="cr_cast"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Childhood Asthma History"] = {"raw_value": cr_cast, "unit": "None"}

                cr_bmi = st.number_input(
                    "Body Mass Index (BMI)", min_value=12.0, max_value=60.0, value=24.0, step=0.1, format="%.1f", key="cr_bmi"
                )
                st.caption("Expected Clinical Range: 12.0 – 60.0 kg/m²")
        ...
        # ======================================================================
            ...
            with st.expander("🧠 Neuropsychiatric & Cognitive Assessment Batteries", expanded=True):
                mh_phq9 = st.number_input(
                    "Patient Health Questionnaire-9 (PHQ-9)", min_value=0, max_value=27, value=4, step=1, key="mh_phq9"
                )
                ...
                mh_gad7 = st.number_input(
                    "Generalized Anxiety Disorder-7 (GAD-7)", min_value=0, max_value=21, value=3, step=1, key="mh_gad7"
                )
                ...
                mh_mmse = st.number_input(
                    "Mini-Mental State Examination (MMSE)", min_value=0, max_value=30, value=28, step=1, key="mh_mmse"
                )
                st.caption("Clinical Scale Range: 0 – 30 Score")
                payload_buffer["Mini-Mental State Examination (MMSE)"] = {"raw_value": mh_mmse, "unit": "Score"}

                mh_moca = st.number_input(
                    "Montreal Cognitive Assessment (MoCA)", min_value=0, max_value=30, value=27, step=1, key="mh_moca"
                )
                st.caption("Clinical Scale Range: 0 – 30 Score")
                payload_buffer["Montreal Cognitive Assessment (MoCA)"] = {"raw_value": mh_moca, "unit": "Score"}

                mh_hamd = st.number_input(
                    "Hamilton Depression Rating Scale (HAM-D)", min_value=0, max_value=52, value=6, step=1, key="mh_hamd"
                )
                ...
                mh_updrs = st.number_input(
                    "UPDRS Part III (Motor Examination)", min_value=0, max_value=132, value=8, step=1, key="mh_updrs"
                )
                st.caption("Clinical Scale Range: 0 – 132 Score")
                payload_buffer["UPDRS Part III (Motor Examination)"] = {"raw_value": mh_updrs, "unit": "Score"}

                mh_faq = st.number_input(
                    "Functional Activities Questionnaire (FAQ)", min_value=0, max_value=30, value=2, step=1, key="mh_faq"
                )
                st.caption("Clinical Scale Range: 0 – 30 Score")
                payload_buffer["Functional Activities Questionnaire (FAQ)"] = {"raw_value": mh_faq, "unit": "Score"}

                mh_trem = st.selectbox(
                    "Tremor Rating Scale Score", options=[0, 1, 2, 3, 4], index=0, key="mh_trem"
                )
            ...
            with st.expander("💤 Sleep Physiology, Somnolence & Paroxysmal Events", expanded=True):
                mh_sleep = st.number_input(
                    "Daily Sleep Duration", min_value=1.0, max_value=16.0, value=7.5, step=0.5, format="%.1f", key="mh_sleep"
                )
                st.caption("Expected Clinical Range: 1.0 – 16.0 Hours")
                payload_buffer["Daily Sleep Duration"] = {"raw_value": mh_sleep, "unit": "Hours"}

                mh_psqi = st.number_input(
                    "Pittsburgh Sleep Quality Index (PSQI)", min_value=0, max_value=21, value=4, step=1, key="mh_psqi"
                )
                st.caption("Clinical Scale Range: 0 – 21 Score")
                payload_buffer["Pittsburgh Sleep Quality Index (PSQI)"] = {"raw_value": mh_psqi, "unit": "Score"}

                mh_seiz = st.number_input(
                    "Seizure Frequency", min_value=0, max_value=100, value=0, step=1, key="mh_seiz"
                )
            ...
            with st.expander("🧪 Endocrine, Autonomic & Neuro-Nutritional Panel", expanded=True):
                mh_hrv = st.number_input(
                    "Heart Rate Variability (SDNN)", min_value=5.0, max_value=250.0, value=65.0, step=1.0, format="%.1f", key="mh_hrv"
                )
                st.caption("Expected Clinical Range: 5.0 – 250.0 ms")
                payload_buffer["Heart Rate Variability (SDNN)"] = {"raw_value": mh_hrv, "unit": "ms"}

                mh_cort = st.number_input(
                    "8 AM Serum Cortisol", min_value=1.0, max_value=45.0, value=14.5, step=0.5, format="%.1f", key="mh_cort"
                )
                st.caption("Expected Clinical Range: 1.0 – 45.0 µg/dL")
                payload_buffer["8 AM Serum Cortisol"] = {"raw_value": mh_cort, "unit": "µg/dL"}

                mh_b12 = st.number_input(
                    "Serum Vitamin B12", min_value=50.0, max_value=2000.0, value=450.0, step=10.0, format="%.1f", key="mh_b12"
                )
                st.caption("Expected Clinical Range: 50 – 2000 pg/mL")
                payload_buffer["Serum Vitamin B12"] = {"raw_value": mh_b12, "unit": "pg/mL"}

                mh_fol = st.number_input(
                    "Serum Folate", min_value=1.0, max_value=25.0, value=7.5, step=0.1, format="%.1f", key="mh_fol"
                )
                st.caption("Expected Clinical Range: 1.0 – 25.0 ng/mL")
                payload_buffer["Serum Folate"] = {"raw_value": mh_fol, "unit": "ng/mL"}

                mh_tsh = st.number_input(
                    "Thyroid Stimulating Hormone (TSH)", min_value=0.01, max_value=100.0, value=1.85, step=0.05, format="%.2f", key="mh_tsh"
                )
            ...
            with st.expander("🧬 Neurological History & Chronic Trauma Markers", expanded=True):
                mh_tbi = st.radio(
                    "History of Traumatic Brain Injury (TBI)", options=["0", "1"], index=0, horizontal=True, key="mh_tbi"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["History of Traumatic Brain Injury (TBI)"] = {"raw_value": mh_tbi, "unit": "None"}

                mh_alc = st.radio(
                    "Lifetime History of Alcohol/Substance Abuse", options=["0", "1"], index=0, horizontal=True, key="mh_alc"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Lifetime History of Alcohol/Substance Abuse"] = {"raw_value": mh_alc, "unit": "None"}

                mh_fam = st.radio(
                    "Family History of Neurological/Psychiatric Illness", options=["0", "1"], index=0, horizontal=True, key="mh_fam"
                )
                st.caption("Input Basis: 0 = Negative / No, 1 = Positive / Yes")
                payload_buffer["Family History of Neurological/Psychiatric Illness"] = {"raw_value": mh_fam, "unit": "None"}

                mh_age = st.number_input(
                    "Age", min_value=18, max_value=90, value=int(active_pat.get("age", 58)), step=1, key="mh_age"
                )
                st.caption("Expected Clinical Range: 18 – 90 Years")
        ...
        # ----------------------------------------------------------------------
        # SUBMISSION & INGESTION CONTRACT DISPATCH
        # ----------------------------------------------------------------------
        submit_inference = st.form_submit_button(
        ...
        if submit_inference:
            ...
            generated_encounter_id = f"ENC-{uuid.uuid4().hex[:8].upper()}"
            submission_timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

            cdss_package = {
                "encounter_id": generated_encounter_id,
            ...
            st.success(f"Biomarkers verified. Dispatching payload for {active_pat['name']} ({generated_encounter_id}).")

    # --------------------------------------------------------------------------
    # SERIALIZED OUTPUT CONTRACT INSPECTION (TELEMETRY VIEW)
    # --------------------------------------------------------------------------
    if st.session_state.get("ai_inference_completed") and st.session_state.get("cdss_inference_payload"):
        st.markdown("<div style='height: 16px;'></div>", unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown("##### 📦 Serialized Gateway Contract Output (`POST /api/models/predict`)")
            st.caption("The exact JSON payload constructed and validated across physiological bounds.")
            st.json(st.session_state["cdss_inference_payload"], expanded=False)



# ==============================================================================
# AI DIAGNOSTIC INFERENCE RESULTS & RISK FINDINGS
# ==============================================================================

# Simulated AI Model Output (Replace with response from POST /api/models/predict)
inference_results = {
    "risk_score": 82.4,  # Overall percentage (0 - 100)
    "risk_tier": "High Risk",  # Low Risk, Moderate Risk, High Risk
    "confidence": 94.2,
    "primary_condition": "Cardiovascular Disease / Stage 2 HTN",
    "drivers": [
        {"feature": "Systolic Blood Pressure (165 mmHg)", "impact": 32.5, "severity": "High"},
        {"feature": "Low-Density Lipoprotein (180 mg/dL)", "impact": 24.0, "severity": "High"},
        {"feature": "Smoking History (25 Pack-Years)", "impact": 18.2, "severity": "High"},
        {"feature": "Sedentary Activity (45 mins/wk)", "impact": 14.1, "severity": "Moderate"},
        {"feature": "Resting Heart Rate (92 bpm)", "impact": 11.2, "severity": "Moderate"},
    ],
}
...
with st.container(border=True):
    st.markdown(
        ...
        <div style="margin-bottom: 12px;">
            <h3 style="margin: 0; font-size: 1.35rem; font-weight: 700;">Diagnostic Risk Assessment & Biomarker Findings</h3>
            <p style="margin: 2px 0 0 0; font-size: 0.9rem; color: #64748b;">Multi-factorial risk stratification and feature attribution generated from verified clinical inputs.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    gauge_col, drivers_col = st.columns([1, 1.4], gap="large")

    # --------------------------------------------------------------------------
    # 1. OVERALL RISK PROBABILITY GAUGE
    # --------------------------------------------------------------------------
    with gauge_col:
        st.markdown("##### Overall Diagnostic Risk Score")

        score = inference_results["risk_score"]
        tier = inference_results["risk_tier"]
        ...
        fig_gauge = go.Figure(
            go.Indicator(
                mode="gauge+number",
                value=score,
                number={"suffix": "%", "font": {"size": 42, "color": tier_color, "family": "Inter, sans-serif"}},
                title={
                    "text": f"<b style='color:{tier_color}; font-size:17px;'>{tier.upper()} DETECTED</b><br><span style='font-size:12px; color:#64748b;'>Confidence: {inference_results['confidence']}%</span>",
                    "font": {"size": 15},
                },
                gauge={
        ...
        st.markdown(
            f"""
            <div style="text-align: center; font-size: 0.88rem; font-weight: 600; padding: 6px 12px; background: rgba(239, 68, 68, 0.1); border-radius: 8px; color: {tier_color};">
                Primary Indication: {inference_results['primary_condition']}
            </div>
            """,
            unsafe_allow_html=True,
        )
    # --------------------------------------------------------------------------
    # 2. FEATURE ATTRIBUTION / TOP RISK DRIVERS (HORIZONTAL BAR CHART)
    # --------------------------------------------------------------------------
    with drivers_col:
        st.markdown("##### Key Contributors to Clinical Risk")
        st.caption("Relative weight (%) of abnormal biomarkers driving the diagnostic probability.")

        drivers = inference_results["drivers"]
        features = [d["feature"] for d in reversed(drivers)]
        impacts = [d["impact"] for d in reversed(drivers)]
        colors = ["#ef4444" if d["severity"] == "High" else "#f59e0b" for d in reversed(drivers)]
